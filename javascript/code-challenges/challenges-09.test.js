...
const checkValues = (obj, value) => {
  ...
  let find = valuesArray.find((element) => element === value);
...
const hasChildrenValues = (arr, character) => {
  for (let object of arr) {
    if (Object.values(object)[0] === character) {
      if (Array.isArray(Object.values(object)[2])) {
        return true;
      } else {
        return false;
      }
...
const totalCharacters = (arr) => {
  let totalCharacters1 = 0;
  for (let i = 0; i < arr.length; i++) {
    totalCharacters1 += arr[i].toString().length;
  }
  return totalCharacters1;
};
...
const houseSize = (arr) => {
  const sizes = [];
  arr.forEach((element) => {
    sizes.push({ house: element.house, members: 3 });
  });
  return sizes;
};
