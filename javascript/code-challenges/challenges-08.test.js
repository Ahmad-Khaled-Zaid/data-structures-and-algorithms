...
const sortByChildren = (charArray) => {
  // Solution code here...
  let newArr = charArray.sort((element1, element2) => {
    ...
    if (element1.children.length === element2.children.length) {
      if (element1.house > element2.house) {
        return 1;
      }
      if (element1.house < element2.house) {
        return -1;
...
const isCapitalized = (str) => {
  // Solution code here...
  let newArr = [];
  let regularExpression = /\b[A-Z](\w)*/g;
  newArr = str.match(regularExpression);
  if (newArr === null) {
    newArr = [];
  }
  return newArr;
