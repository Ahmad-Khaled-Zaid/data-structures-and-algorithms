...
const totalSum = (input) => {
  // Solution code here...
  let summation = input.reduce((acc, n) => {
    n.map((item) => {
      acc = acc + item;
      return acc;
    });
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArray = input.map((element) => {
    if (element) {
      let newArr = element.filter((n) => typeof n == "number" && n % 5 == 0);
      return newArr.map((m) => {
        return Math.pow(2, m);
      });
    } else if (!element) {
...
let findShortest = (data) => {
  // Solution code here...
  let sorted = data.sort((element1, element2) =>
